...
def show_area_distribution(friends):
    # 使用一个字典统计各省好友数量
    province_dict = {'北京': 0, '上海': 0, '天津': 0, '重庆': 0, '澳门': 0,
                     '河北': 0, '山西': 0, '吉林': 0, '辽宁': 0, '黑龙江': 0,
                     '陕西': 0, '甘肃': 0, '青海': 0, '山东': 0, '福建': 0,
                     '浙江': 0, '台湾': 0, '河南': 0, '湖北': 0, '湖南': 0,
                     '江西': 0, '江苏': 0, '安徽': 0, '广东': 0, '海南': 0,
                     '四川': 0, '贵州': 0, '云南': 0, '内蒙古': 0, '新疆': 0,
                     '宁夏': 0, '广西': 0, '西藏': 0, '香港': 0, '其他': 0}
    # 统计省份
    for friend in friends:
...
def show_signature(friends):
    ...
    for friend in friends:
        ...
        print('{}\n[{}]\n'.format(friend.name, friend.signature))
    ...
    wordcloud = WordCloud(font_path='simhei.ttf',          # 设置字体可以显示中文
                          ...
                          random_state=42,
                          # 设置图片默认的大小,但是如果使用背景图片的话, 那么保存的图片大小将会按照其大小保存, margin为词语边缘距离                                                                                                    # 那么保存的图片大小将会按照其大小保存,margin为词语边缘距离
                          width=651, height=462, margin=2)
